...
export function LanguageSwitcher() {
  ...
  const { open, setOpen, close, containerRef, triggerRef, setItemRef, onTriggerKeyDown, onMenuKeyDown } =
    useMenu(SUPPORTED_LOCALES.length);

  function handleSelect(next: Locale) {
    // No focus return: the page is about to reload entirely, so the
    // trigger this focus would land on is about to be replaced anyway.
    close(false);
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        {/* aria-hidden: the globe is decorative here - the button already
            has an explicit accessible name via aria-label, so announcing
            the icon too would just be noise. */}
      ...
      {open && (
        ...
        <div
          ...
          className="absolute left-0 top-full mt-2 max-h-80 w-44 overflow-y-auto rounded-md border border-border bg-surface py-1 text-sm shadow-md"
        >
          {SUPPORTED_LOCALES.map((code, index) => (
            <button
              ...
              // Each option is written in its own language (LOCALE_LABELS
              // are endonyms - "日本語", not "Japanese"), so without this
              // a screen reader would read every one of them through the
              // current interface language's voice and pronunciation
              // rules. lang lets it switch voices per item, which is what
              // makes this menu usable for the very people most likely to
              // need it: someone who can't read the current interface
              // language looking for their own.
              lang={code}
